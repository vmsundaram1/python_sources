# This program calculates the distance for a N dimensional co-ordinates represented in Vector Form. It used Functions and While Loop.

import math

# This Function computes the the square root and returns the value
...
# This Function calculates the vector distance by passing two input vectors containing the N dimensional co-ordinate values


def Calculate_NDimensional_Vector_Distance(count1,Arr1, Arr2):
 i=0
 size=count1
 print("Count =",count1)
 distance=0.0
...
def input_collection_and_processing():
 ...
 count = input("Enter the number of dimensions for the 2 Vectors/Arrays: ")
...
# Check the Collected Arrays/Vectors, format
 ...
 while(not done):
  ...
  if(len(array_one)!=len(array_two) or len(array_one)!=int(count) or len(array_two)!=int(count)):
   done=False
   print("Input Data Error: You need the enter correct input data again!")
   input_collection_and_processing()
  done=True
 
 distance_value = Calculate_NDimensional_Vector_Distance(int(count),array_one,array_two)
 print(distance_value)
 print("The distance between the 2 N-dimensional vectors is: ", round(distance_value,4))
